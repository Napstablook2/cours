...
#let thm(title:"", text, ..other) = [#block(
  fill: luma(230),
  inset: 8pt,
  radius: 4pt,
  width: 100%,
  [*Théorème#title (#context counter("thm").step()#context counter("thm").display())*: #text]
)]
...
#let exemple(text, ..other) = [#block(
  fill: luma(98.04%),
  inset: 8pt,
  radius: 4pt,
  width: 100%,
  [*Exemple #context counter("exemple").step()#context counter("exemple").display()*: #text]
)]
#let ul(text, ..other) = underline[#text]
#let ol(text, ..other) = overline[#text]
